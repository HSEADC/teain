...
/* harmony default export */ const anime_es = (anime);
...
;// CONCATENATED MODULE: ./src/js/typesoftee/fermentation.js
...
jquery(function () {
  var sliderButtons = document.querySelectorAll('.A_SliderButton');
  sliderButtons.forEach(function (item) {
    var bg = item.querySelector('#bg');
  ...
  jquery('#searchBtn').mouseenter(function () {
    jquery('.M_SearchIcon').addClass('active').focus();
    jquery('.A_SearchInput').addClass('active');
  });
  jquery('.M_SearchIcon').mouseleave(function () {
    if (jquery('.A_SearchInput').val().trim() === '') {
      jquery('.M_SearchIcon').removeClass('active');
      jquery('.A_SearchInput').removeClass('active').blur();
      jquery('.W_TeaType').show();
      jquery('.O_PreArticle').show();
      jquery('.W_TextLink').show();
      jquery('.A_Lead').show();
      jquery('.O_SearchResult').hide();
    }
  });
  jquery('.A_SearchInput').on('input', function () {
    jquery('.W_TeaType').hide();
    jquery('.O_PreArticle').hide();
    jquery('.W_TextLink').hide();
    jquery('.A_Lead').hide();
    jquery('.O_SearchResult').show();
    var searchText = jquery(this).val().toLowerCase().trim();
    var searchWords = searchText.split(/\s+/);
    var found = false;
    jquery('.M_TeaCard').each(function () {
      var cardText = jquery(this).find('.A_CardTitleHelios').text().toLowerCase() + ' ' + jquery(this).find('.A_CardTitleVlas').text().toLowerCase();
    ...
    if (found) {
      jquery('.A_SearchResult').html('по запросу "<span id="searchText">' + searchText + '</span>":');
    } else {
      jquery('.A_SearchResult').html('по запросу "<span id="searchText">' + searchText + '</span>" ничего не найдено');
    }
  });
});
})();
